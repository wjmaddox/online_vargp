...
def main(
    ...
    mll_gibbon, model_gibbon = initialize_model_poisson(
    ...
    for iteration in range(1, n_batch + 1):
        ...
        qEI = qExpectedImprovement(
            model=model_ei,
            best_f=(
                (train_obj_ei + 0.001).log() * (train_con_ei <= 0).to(train_obj_ei)
            ).max(),
            sampler=qmc_sampler,
