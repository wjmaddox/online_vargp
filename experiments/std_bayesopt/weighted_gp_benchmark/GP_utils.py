...
def SPGP_likelihood(params, y, x, m, compute_deriv=True):
    ...
    partial += float(np.dot(y.transpose(), y) - np.dot(bet.transpose(), bet)) / (
        2 * sigma
    )
    ...
    for i in range(dim):
        ...
        dARD = dARD - np.dot(
            np.reshape(np.sum(dNnK, axis=0), (1, N)), np.reshape(x[:, i], (N, 1))
        )
    ...
    ep = ep.transpose()
    epc = (
        coeff / ep - sumVsq - jitter * np.reshape(np.sum(invLV * invLV, axis=0), (N, 1))
    ) / sigma
