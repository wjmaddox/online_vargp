...
import pandas as pd
from upcycle.plotting.credible_regions import combine_trials

def get_gaussian_region(mean, variance, ntrials):
    lb = mean - 2 * (variance**0.5) / (ntrials ** 0.5)
    ub = mean + 2 * (variance**0.5) / (ntrials ** 0.5)
    return mean, lb, ub
...
def get_arm(exp_dir, arm_name, table_name, x_col, y_col, window=1):
    ...
    mean, lb, ub = get_gaussian_region(arm_data.mean(-1), arm_data.var(-1), arm_data.shape[-1])
